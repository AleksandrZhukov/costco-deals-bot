...
# Using jq to parse JSON (if available), otherwise fallback to basic parsing
if command -v jq &> /dev/null; then
  ...
  for ((i=0; i<$stories_count; i++)); do
    ...
    if [ "$status" = "completed" ]; then
      ...
      echo "Skipping completed story: $story_id"
      continue
    fi
  ...
  echo "jq not found. Please install jq: brew install jq"
  echo "Or manually parse the JSON file"
  exit 1
fi
